...
export class CitiesNote extends Component<{ navigation: any, route: any }, CitiesNotesState>{
    ...
    async componentDidMount() {
        const storedNotes = await AsyncStorage.getItem(`cityNotes${this.state.character._id}`)
        if (storedNotes) {
    ...
    deleteNote = async (noteToDel: string) => {
        ...
        if (storedNotes) {
            ...
            await AsyncStorage.setItem(`cityNotes${this.state.character._id}`, JSON.stringify(newNotes));
    ...
    updateNote = async (values: any) => {
        ...
        if (storedNotes) {
            ...
            await AsyncStorage.setItem(`cityNotes${this.state.character._id}`, JSON.stringify(oldNotes));
    ...
    render() {
        const imgArray = ['castle.png', 'castleII.png', 'villageI.png', 'city.png', 'villageII.png']
        return (
            <View>
                {this.state.loading ? <AppActivityIndicator visible={this.state.loading} /> :
                    <View style={styles.container}>
                        <AppText textAlign={'center'} fontSize={25} color={Colors.bitterSweetRed}>Cities and Villages</AppText>
                        <AppText textAlign={'center'} fontSize={17}>These are your personal notes regarding cities and villages you have discovered during your adventures</AppText>
                                    ...
                                    <AppForm
                                        ...
                                        <View >
                                            <AppFormField
                                                ...
                                                placeholder={"Name of the city or village..."} />
                                        ...
                                        <View style={{ justifyContent: "center", alignItems: "center", padding: 20, height: 160 }}>
                                            <AppText fontSize={18} textAlign={'center'}>If you wish you can pick an Icon to better visualize this place.</AppText>
                                            <ScrollView horizontal style={{ flexDirection: 'row', width: Dimensions.get('screen').width / 1.1 }}>
                                                {imgArray.map((item, index) => <TouchableOpacity key={index} onPress={() => {
                                                    ...
                                                    <View style={{ alignItems: "center", backgroundColor: this.state.pickedImgShown[index] ? Colors.bitterSweetRed : Colors.white, height: 70, width: 70, borderRadius: 110, borderWidth: 1, borderColor: Colors.whiteInDarkMode }}>
                                                        <Image style={{ height: 60, width: 60 }} uri={`${Config.serverUrl}/assets/logos/${item}`} />
                                                    </View>
                                                </TouchableOpacity>)}
                                            </ScrollView>
                                        </View>
